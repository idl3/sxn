  ...
  module Config
    ...
    class ConfigValidator
      ...
      def migrate_config(config)
        ...
        needs_v0_migration = version == 0 || needs_v0_to_v1_migration?(config)
      ...
      def needs_v0_to_v1_migration?(config)
        ...
        config['projects'].any? do |project_name, project_config|
      ...
      def validate_field_type(value, schema, path)
        ...
        unless value_has_correct_type?(value, schema)
      ...
      def validate_field_constraints(value, schema, path)
        ...
        if value.is_a?(Integer)
          if schema[:min] && value < schema[:min]
            @errors << "Field '#{path}' must be at least #{schema[:min]}"
          end
          
          if schema[:max] && value > schema[:max]
            @errors << "Field '#{path}' must be at most #{schema[:max]}"
          end
        end
        ...
        if schema[:allowed_values] && !schema[:allowed_values].include?(value)
      ...
      def validate_nested_schema(value, schema, path)
        ...
        if schema[:item_schema] && value.is_a?(Array)
          value.each_with_index do |item, index|
            ...
            if item.is_a?(Hash)
              validate_against_schema(item, schema[:item_schema], item_path)
            end
          end
        end
          ...
          if is_structured && (path.include?('settings') || path.include?('rules') || path.include?('default_rules'))
            ...
            value.each do |key, nested_value|
              ...
              if nested_value.is_a?(Hash)
                validate_against_schema(nested_value, schema[:value_schema], nested_path)
              end
            end
          end
        end
        ...
        if schema[:value_type] && value.is_a?(Hash)
          value.each do |key, nested_value|
            nested_path = "#{path}.#{key}"
      ...
      def apply_defaults_recursive(config, schema, root_config)
        schema.each do |key, field_schema|
          ...
          if config[key].nil? && field_schema.key?(:default)
            config[key] = field_schema[:default].dup rescue field_schema[:default]
          end
          ...
          if config[key].is_a?(Hash) && field_schema[:value_schema]
            ...
            if field_schema[:type] == :hash
              ...
              config[key].each do |nested_key, nested_value|
                if nested_value.is_a?(Hash)
          ...
          if config[key].is_a?(Array) && field_schema[:item_schema]
            config[key].each do |item|
              if item.is_a?(Hash)
                apply_defaults_recursive(item, field_schema[:item_schema], root_config)
              end
            end
          end
        end
      ...
      def migrate_v0_to_v1(config)
        ...
        if migrated.key?('auto_cleanup')
          migrated['settings']['auto_cleanup'] = migrated.delete('auto_cleanup')
        end
        
        if migrated.key?('max_sessions')
          migrated['settings']['max_sessions'] = migrated.delete('max_sessions')
        end
        ...
        if migrated['projects'].is_a?(Hash)
          migrated['projects'].each do |project_name, project_config|
            ...
            if project_config['rules']
              migrate_rules_v0_to_v1(project_config['rules'])
            end
          end
        end
      ...
      def migrate_rules_v0_to_v1(rules)
        ...
        if rules['setup_commands'].is_a?(Array)
