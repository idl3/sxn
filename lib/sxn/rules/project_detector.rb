    ...
    class ProjectDetector
      ...
      def detect_project_type
        ...
        PROJECT_TYPES.each do |type, criteria|
          ...
          detected_types << { type: type, confidence: confidence } if confidence > 0
        end

        # Sort by confidence and return the highest
        detected_types.sort_by { |t| -t[:confidence] }.first&.fetch(:type) || :unknown
      end
      ...
      def detect_package_manager
        PACKAGE_MANAGERS.each do |manager, criteria|
          if criteria[:files].any? { |file| file_exists_in_project?(file) }
            return manager
          end
        end
      ...
      def validate_project_path!
        ...
        unless File.readable?(@project_path)
      ...
      def detect_database
        ...
        databases << :postgresql if file_contains?("config/database.yml", "postgresql") || file_contains?("DATABASE_URL", "postgres")
        databases << :mysql if file_contains?("config/database.yml", "mysql") || file_contains?("DATABASE_URL", "mysql")
      ...
      def suggest_setup_commands_rules(project_info)
        ...
        when :bundler
          commands << { "command" => ["bundle", "install"], "description" => "Install Ruby dependencies" }
          if project_info[:type] == :rails
            commands << { "command" => ["bin/rails", "db:create"], "condition" => "file_missing:db/development.sqlite3", "description" => "Create database" }
            commands << { "command" => ["bin/rails", "db:migrate"], "description" => "Run database migrations" }
          end
        when :npm
          commands << { "command" => ["npm", "install"], "description" => "Install Node.js dependencies" }
          commands << { "command" => ["npm", "run", "build"], "condition" => "file_exists:package.json", "required" => false, "description" => "Build project" }
        when :yarn
          commands << { "command" => ["yarn", "install"], "description" => "Install Node.js dependencies" }
          commands << { "command" => ["yarn", "build"], "condition" => "file_exists:package.json", "required" => false, "description" => "Build project" }
        when :pnpm
          commands << { "command" => ["pnpm", "install"], "description" => "Install Node.js dependencies" }
        when :pip
          commands << { "command" => ["pip", "install", "-r", "requirements.txt"], "description" => "Install Python dependencies" }
        when :pipenv
          commands << { "command" => ["pipenv", "install"], "description" => "Install Python dependencies" }
        when :poetry
          commands << { "command" => ["poetry", "install"], "description" => "Install Python dependencies" }
        end
      ...
      def file_contains?(file_path, content)
        ...
        File.read(full_path).include?(content)
      end
      ...
      def analyze_directory_structure
        ...
        found_dirs = important_dirs.select do |dir|
      ...
      def analyze_dependencies
        ...
        if file_exists_in_project?("Gemfile.lock")
          deps[:ruby] = parse_gemfile_lock
        end

        # Node.js dependencies  
        if file_exists_in_project?("package.json")
          deps[:nodejs] = parse_package_json
        end

        # Python dependencies
        if file_exists_in_project?("requirements.txt")
          deps[:python] = parse_requirements_txt
        end
      ...
      def analyze_configuration_files
        config_files = Dir.glob(File.join(@project_path, "**", "*.{yml,yaml,json,toml,ini,conf,config}"))
      ...
      def analyze_scripts
        ...
        if file_exists_in_project?("package.json")
          begin
            ...
            scripts[:npm] = package_data.dig("scripts")&.keys || []
          rescue JSON::ParserError
      ...
      def analyze_documentation
        doc_files = Dir.glob(File.join(@project_path, "**", "*.{md,txt,rst,adoc}"))
          .map { |f| Pathname.new(f).relative_path_from(Pathname.new(@project_path)).to_s }
          .select { |f| !f.start_with?("node_modules/") && !f.start_with?(".git/") }
