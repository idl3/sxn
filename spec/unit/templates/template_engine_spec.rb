...
RSpec.describe Sxn::Templates::TemplateEngine do
  ...
  before do
    ...
    allow(mock_variables_collector).to receive(:collect).and_return({
      session: { name: "test-session", path: "/tmp/test-session" },
      git: { branch: "main" },
      user: { name: "Test User" },
      sxn: { version: "1.0.0" }
    })
  ...
  describe "#process_template" do
    ...
    it "refuses to overwrite existing files without force option" do
      ...
      }.to raise_error(Sxn::Templates::Errors::TemplateError, /already exists/)
    end
  ...
  describe "#template_info" do
    ...
    it "returns template metadata" do
      ...
      allow(mock_processor).to receive(:extract_variables).and_return(["user", "git"])
  ...
  describe "template finding" do
    ...
    it "handles missing templates gracefully" do
      ...
      }.to raise_error(Sxn::Templates::Errors::TemplateNotFoundError, /not found/)
    end

    it "provides helpful error messages for missing templates" do
      expect {
        engine.send(:find_template, "missing/template", nil)
      }.to raise_error(Sxn::Templates::Errors::TemplateNotFoundError, /Available templates/)
    end
  end
  ...
  describe "error handling" do
    it "wraps template processing errors with context" do
      ...
      }.to raise_error(Sxn::Templates::Errors::TemplateProcessingError, /Failed to process template 'test'/)
    end
  end
