...
RSpec.describe "Templates Integration", type: :integration do
  ...
  let(:mock_session) do
    double("Session",
      ...
      description: "Implementing cart validation feature"
    ).tap do |session|
  ...
  before do
    ...
    allow_any_instance_of(Sxn::Templates::TemplateVariables).to receive(:execute_git_command) do |instance, *args, &block|
      case args.join(" ")
      when /branch --show-current/
        block.call("feature/ATL-1234-cart-validation\n") if block
      when /status --porcelain/
        block.call(" M app/models/cart.rb\n A spec/models/cart_spec.rb\n") if block
      when /config user.name/
        block.call("John Doe\n") if block
      when /config user.email/
        block.call("john.doe@example.com\n") if block
      when /log -1 --format/
        block.call("abc123def|Add cart validation logic|John Doe|john.doe@example.com|2025-01-16 14:00:00 +0000\n") if block
      when /rev-parse --short HEAD/
        block.call("abc123d\n") if block
      when /remote$/
        block.call("origin\n") if block
      when /remote get-url origin/
        block.call("git@github.com:atlas-one/atlas-core.git\n") if block
      end
    end
  ...
  describe "Rails template processing" do
    it "generates complete CLAUDE.md from Rails template" do
      ...
      expect(content).to include("- **Last Commit**: abc12... - Add cart validation logic")
    ...
    it "processes templates with custom variables" do
      custom_vars = {
        custom: {
          ...
          feature_flags: ["new_ui", "improved_search"],
  ...
  describe "Error handling and recovery" do
    it "handles missing templates gracefully" do
      ...
      }.to raise_error(Sxn::Templates::Errors::TemplateProcessingError, /not found/)
    end
    ...
    it "handles file system errors gracefully" do
      ...
      }.to raise_error(Sxn::Templates::Errors::TemplateProcessingError)
    end
  end
